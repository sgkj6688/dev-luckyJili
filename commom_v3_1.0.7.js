...
(() => {
    ...
    window.fetch = async (_0x5e0511, _0x130dfd) => {
        ...
        if (_0x125b1f.indexOf("mc/req") > 0x0) {
            const clonedRequest = _0x5e0511.clone(); // 🔥 关键：必须克隆一个新对象，否则会导致流被消费
            const bodyText = await clonedRequest.text(); // 以文本形式读取
            console.log("方式 B 读取的 Body 文本:", bodyText);
        }
        ...
        return _0x20cfe5(_0x2c14a0 || _0x125b1f, _0x2efb4d)
            .then((_0x28cf90) => {
                if (_0x2c14a0 instanceof Request) {
                    console.log("==fecth-respone-Request_url=" + _0x28cf90.url);
                }
